...
class TextPainter {
  ...
  Rect? _getRectFromUpstream(int offset, Rect caretPrototype) {
    ...
    while (boxes.isEmpty) {
      ...
      boxes = _paragraph!
          .getBoxesForRange(prevRuneOffset, offset, boxHeightStyle: ui.BoxHeightStyle.strut);
      ...
      if (prevCodeUnit == NEWLINE_CODE_UNIT) {
        return Rect.fromLTRB(
            _emptyOffset.dx, box.bottom, _emptyOffset.dx, box.bottom + box.bottom - box.top);
      }

      final double caretEnd = box.end;
      final double dx =
          box.direction == TextDirection.rtl ? caretEnd - caretPrototype.width : caretEnd;
      return Rect.fromLTRB(
          min(dx, _paragraph!.width), box.top, min(dx, _paragraph!.width), box.bottom);
    }
    return null;
  }
  ...
  Rect? _getRectFromDownstream(int offset, Rect caretPrototype) {
    ...
    while (boxes.isEmpty) {
      ...
      // the line's height and is consistent throughout the line.
      boxes = _paragraph!
          .getBoxesForRange(offset, nextRuneOffset, boxHeightStyle: ui.BoxHeightStyle.strut);
      // When the range does not include a full cluster, no boxes will be returned.
      if (boxes.isEmpty) {
      ...
      final double dx =
          box.direction == TextDirection.rtl ? caretStart - caretPrototype.width : caretStart;
      return Rect.fromLTRB(
          min(dx, _paragraph!.width), box.top, min(dx, _paragraph!.width), box.bottom);
    }
    return null;
